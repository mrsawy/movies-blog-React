...
const Similar = (props) => {
  ...
  return (
    <>
    <div className={classes.similar_title}> Similar Movies</div>
      <section className={classes.similar_section}>
        {props.kind==="movie" && similarMovies
          ? similarMovies.map((e) => (
            <Link 
            key={e.id}
            style={{color:`black`}} to={`/${props.kind === `movie`?`movies`:`tv`}/${e.id}`}>
              <Card
          ...
          ? similarMovies.map((e,i) => (
              <Card
                ...
                className={`${classes.card_style} ${ i===0? classes.margin:``} `}
                title={
                ...
                date={
                  e.first_air_date ? parseFloat(e.first_air_date).toFixed(1) : ``
                }
                rate={parseFloat(e.vote_average ).toFixed(1)}
                img={`https://www.themoviedb.org/t/p/w220_and_h330_face${e.poster_path}`}
              />
      ...
      <hr className={classes.similar_hr} />
    </>
  );
};
